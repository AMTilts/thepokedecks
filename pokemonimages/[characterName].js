...
export default function Character ({ data, character }) {
    // console.log({data});
    ...
    function gotoShiny(gotoDefault) {
        ...
        setCurrentPosition3(`${sprites.back_shiny}`);
        setCurrentPosition4(`${sprites.back_default}`);
        setCurrentCaption('Shiny Variant');
...
{/* export async function getStaticProps({params}) {
    const results = await fetch(`https://pokeapi.co/api/v2/pokemon/${params.character.name}`).then(res => res.json())
    return {
        props: {
            character: results[0],
                pokeName: results[0]
...
export async function getStaticPaths() {
    const pokemonList = await fetch('https://pokeapi.co/api/v2/pokemon/').then(res => res.json())
    return {
        paths: [
